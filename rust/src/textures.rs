...
const LUT_TEXTURE_VERTEX_SHADER: &str = r#"
#version 330 core
...
const LUT_TEXTURE_FRAGMENT_SHADER: &str = r#"
#version 330 core
...
vec2 integrate_brdf(float n_dot_v, float roughness) {
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i) {
        ...
        if(n_dot_l > 0.0) {
            float G = geometry_funciton_smith(n_dot_v, n_dot_l, roughness);
...
pub fn compute_lut_texture(resolution: i32) -> Texture2D {
    ...
    let shader =
        Shader::new_from_source(&LUT_TEXTURE_VERTEX_SHADER, &LUT_TEXTURE_FRAGMENT_SHADER).unwrap();
    shader.bind();
